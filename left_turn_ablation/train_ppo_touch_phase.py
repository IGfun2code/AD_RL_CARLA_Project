...
def main():
    ...
    EnvClass = getattr(env_module, args.env_class)
    env = Monitor(
        EnvClass(
            host=args.host,
            port=args.port,
    ...
    try:
        model = PPO(
            ...
            env,
            verbose=1,
            tensorboard_log=os.path.join(args.log_dir, args.tag),
        ...
        out_path = os.path.join(args.checkpoint_dir, f"ppo_{args.tag}.zip")
        model.save(out_path)
